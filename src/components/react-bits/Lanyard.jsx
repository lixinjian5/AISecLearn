...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false, frontImage = null, backImage = null, imageFit = 'cover', lanyardImage = null, lanyardWidth = 1 }) {
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img, rect) => {
      ...
      ctx.drawImage(img, rx + (rw - img.width * scale) / 2, ry + (rh - img.height * scale) / 2, img.width * scale, img.height * scale);
      ctx.restore();
    };
  ...
  useEffect(() => {
    if (hovered) { document.body.style.cursor = dragged ? 'grabbing' : 'grab'; return () => { document.body.style.cursor = 'auto'; }; }
  }, [hovered, dragged]);
